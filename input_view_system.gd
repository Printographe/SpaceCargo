...
func _ready() -> void:
    ...
    if player and player is PlayerController:
        ...
        on_speed_change(null, player.movement_statemachine.get_state())

func on_speed_change(_past, current):
    if current == PlayerController.STATES.THIRD_GEAR:
        key_x_button.set_disabled(true)
    elif current == PlayerController.STATES.IDLE:
...
func disconnect_interaction(interaction : Interaction):
    ...
    if pertaining_button.pressed.is_connected(interaction.callback):
        pertaining_button.pressed.disconnect(interaction.callback)
...
func set_focus(interaction: Interaction):
    ...
    key_to_button[interaction.key].pressed.connect(interaction.callback)
...
func _input(event: InputEvent) -> void:
    for key in current_focus:
        ...
        if Input.is_action_pressed(int_key_to_ui[interaction.key]):
